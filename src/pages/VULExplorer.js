  ...
  tbody.innerHTML = displayed.map(p => `
    <tr class="border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/30">
      ...
      <td class="px-3 py-2.5 cursor-pointer" onclick="location.hash='/vul/${p.id}'">
        <div class="font-medium text-teal-600 dark:text-teal-400 hover:underline">${p.name}${p.ratesVerified ? ' <span title="Rates verified from carrier source" class="inline-flex items-center ml-1 px-1.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-semibold rounded-full align-middle">Verified</span>' : ''}</div>
      </td>
